...
var RevealAudioSlideshow =
	...
	(function() {
		...
		var playerOpacity = 0.15 // opacity when the mouse is far from to the audioplayer
		...
		function setup() {
			...
			if (config) {
				...
				if (config.textToSpeechURL != null)
					textToSpeechURL = config.textToSpeechURL
			...
			var css =
				".audio-controls>audio { opacity:" +
				playerOpacity +
				";} .audio-controls:hover>audio { opacity:1;}"
		...
		function getText(textContainer) {
			...
			for (var i = 0, len = elements.length; i < len; i++) {
				...
				textContainer.innerHTML = textContainer.innerHTML.replace(
					...
					elements[i].getAttribute("data-audio-text")
				)
			}
		...
		function setupAllAudioElements(container, h, v, slide) {
			...
			setupAudioElement(
				container,
				h + "." + v,
				slide.getAttribute("data-audio-src"),
				text,
				slide.querySelector(":not(.fragment) > video[data-audio-controls]")
			)
			var i = 0
			var fragments
			while (
				(fragments = slide.querySelectorAll(
		...
		function linkVideoToAudioControls(audioElement, videoElement) {
			...
			var videoSilence = new SilentAudio(
				Math.round(videoElement.duration + 0.5)
		...
		function setupAudioElement(
			...
			audioElement.setAttribute(
				"style",
				"position: relative; top: 20px; left: 10%; width: 80%; outline: none;"
			)
			...
			audioElement.addEventListener("ended", function(event) {
				if (typeof Recorder == "undefined" || !Recorder.isRecording) {
					...
					if (typeof indices.f !== "undefined" && indices.f >= 0) {
						var fragment = slide.querySelector(
...
var FastBase64 = {
	...
	Init: function() {
		for (var e = 0; 4096 > e; e++)
			this.encLookup[e] = this.chars[e >> 6] + this.chars[63 & e]
	},
	Encode: function(e) {
		for (var h = e.length, a = "", t = 0; h > 2; )
...
var SilentAudio = function(e) {
	...
	function t(e) {
		for (var h = [], a = 0, t = e.length, s = 0; t > s; s++)
			(h[a++] = 255 & e[s]), (h[a++] = (e[s] >> 8) & 255)
		return h
	}
	;(this.data = []),
		(this.wav = []),
		...
		(this.Make = function(e) {
			...
			;(this.header.blockAlign =
				...
				(this.wav = this.header.chunkId.concat(
					h(this.header.chunkSize),
					this.header.format,
					this.header.subChunk1Id,
					h(this.header.subChunk1Size),
					a(this.header.audioFormat),
					a(this.header.numChannels),
					h(this.header.sampleRate),
					h(this.header.byteRate),
					a(this.header.blockAlign),
					a(this.header.bitsPerSample),
					this.header.subChunk2Id,
					h(this.header.subChunk2Size),
					16 == this.header.bitsPerSample ? t(this.data) : this.data
				)),
